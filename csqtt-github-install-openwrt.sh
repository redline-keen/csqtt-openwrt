#!/bin/sh
# -*- coding: utf-8 -*-
# CSQTT — установка клиентского бинарника с GitHub на роутер OpenWrt
# с Авторежимом ВК (auto_js), пулом хешей и суточной ротацией
# (один хеш в сутки, окно 09:30–15:10, случайный порядок).
#
# Использование:
#   sh csqtt-github-install-openwrt.sh 'csqtt://connect?...' [опции]
#   sh csqtt-github-install-openwrt.sh --repo ВАШ_ЛОГИН/ВАШ_РЕПО ...
#
# Опции:
...
#   --mihomo-conf          скачать csqtt-config.yaml в /etc/mihomo/config.yaml
#   --mihomo-conf-url URL  свой URL конфига mihomo
#   --no-mihomo            не спрашивать про конфиг mihomo
...
#   -h|--help              эта справка
#
# ЗАВИСИМОСТИ OPENWRT (проверяются установщиком, ставятся один раз):
#   opkg update
#   opkg install curl ca-bundle      # либо достаточно uclient-fetch + libustream-*
#   opkg install kmod-tun            # TUN-режим (интерфейс csqtt0), на большинстве
#                                    # прошивок уже есть
#   procd и cron (crond) входят в базовую систему OpenWrt.
#
# ПУТИ OPENWRT:
#   каталог установки:   /etc/csqtt
#   init-скрипт (procd): /etc/init.d/csqtt → service csqtt start|stop|restart|status|log
#   cron:                /etc/crontabs/root (ротация */5 минут, watchdog */2 минуты)
#   конфиг mihomo:       /etc/mihomo/config.yaml (прежний сохраняется как *.csqtt-bak)
#   журнал клиента:      /etc/csqtt/csqtt.log (watchdog держит его в пределах 1 МБ)
#   деинсталлятор:       csqtt-uninstall
#
...
# пинг через него; любой сбой — перезапуск службы. Падения процесса ловит
# и сам procd (respawn), watchdog страхует TUN/пинг. Уважает ручную остановку
# (service csqtt stop создаёт stopped-флаг) и не лезет во время рестарта
# ротацией. Ограничивает лог клиента /etc/csqtt/csqtt.log одним мегабайтом
# (обрезка до 500 последних строк, без подмены файла — клиент держит его
# открытым).
#
# ДЕИНСТАЛЯТОР: команда csqtt-uninstall (или /etc/csqtt/uninstall.sh) —
# останавливает службу, снимает автозапуск (disable), чистит cron-строки
# ротации/watchdog, удаляет init-скрипт и весь каталог установки.
#
# КОНФИГ MIHOMO: по запросу установщика скачивает csqtt-config.yaml из ветки
# main того же GitHub-репозитория (--repo) и кладёт в /etc/mihomo/config.yaml;
# существующий конфиг сохраняется бэкапом *.csqtt-bak. Флаги: --mihomo-conf
# (скачать), --mihomo-conf-url URL (свой URL конфига), --no-mihomo (не спрашивать).

set -u

show_help() {
cat <<'HELP'
CSQTT — установка клиента CSQTT с GitHub на роутер OpenWrt.
...
Опции:
  ...
  --hashes N             хешей в пуле 1..6 (Enter = 4)
  --workers N            воркеры 9..162, максимум хеши×27, кратно 9
  --mihomo-conf          скачать csqtt-config.yaml в /etc/mihomo/config.yaml
  --mihomo-conf-url URL  свой URL конфига mihomo
  --no-mihomo            не спрашивать про конфиг mihomo
  --no-start             установить, но не запускать
  --no-rotate            без cron-ротации хешей
  --no-watchdog          без cron-watchdog

Зависимости (один раз):
  opkg update && opkg install curl ca-bundle kmod-tun
  (curl не обязателен, если работает uclient-fetch с libustream-*)

Управление после установки:
  service csqtt start|stop|restart|status|log
  Журнал клиента: /etc/csqtt/csqtt.log (или service csqtt log)
  Ротация вне очереди: /etc/csqtt/csqtt-rotate-hashes.sh force
  Деинсталляция: csqtt-uninstall
HELP
}
...
mkdir -p "$CSQTT_DIR" /etc/init.d 2>/dev/null \
    || die "нет прав на запись (запускайтесь под root: ssh root@<ip-роутера>)"

# свободное место на overlay (каталог /etc) — бинарник не крошечный
avail_kb=$(df -k /etc 2>/dev/null | awk 'NR==2 {print $4}')
case "$avail_kb" in
    ""|*[!0-9]*) avail_kb="" ;;
esac
if [ -n "$avail_kb" ] && [ "$avail_kb" -lt 20480 ]; then
    warn "На разделе с /etc свободно ${avail_kb} КБ — бинарник может не поместиться"
fi

# сетевой загрузчик для GitHub
FETCHER=""
if command -v curl >/dev/null 2>&1; then
    FETCHER="curl"
elif command -v wget >/dev/null 2>&1; then
    FETCHER="wget"    # на OpenWrt wget — обычно симлинк на uclient-fetch
else
    die "нет ни curl, ни wget — выполните: opkg update && opkg install curl"
fi

# TUN-устройство для csqtt0 (клиент создаёт интерфейс сам, но ядро должно уметь)
if [ ! -c /dev/net/tun ]; then
    warn "/dev/net/tun не найден — TUN-режим не поднимется."
    warn "Лечится так: opkg update && opkg install kmod-tun && insmod tun (или перезагрузка)"
fi

log "Каталог: $CSQTT_DIR · init: $INIT_SCRIPT (procd) · загрузчик: $FETCHER"

# ── 2. архитектура ───────────────────────────────────────────────────────────
ARCH_KEY=""
case "$(uname -m)" in
    aarch64|arm64)
        # 64-бит LE — MT7981, AN7581 и другие ARM-системы-на-чипе
        ARCH_KEY="aarch64" ;;
    mips)
...
hexdump_bin() { # один байт из stdin → две hex-цифры; только POSIX-утилиты
    # путь 1: hexdump (busybox OpenWrt умеет -e)
    if command -v hexdump >/dev/null 2>&1; then
        hexdump -n 1 -e '1/1 "%02x"'
    # путь 2: od классический (есть в busybox OpenWrt)
...
if [ -n "$CSQTT_LOCAL_BIN" ]; then
    ...
    # тег последнего релиза — через GitHub API (не зависит от редиректов,
    # работает и с busybox wget/uclient-fetch, и с curl)
    if [ -z "$CSQTT_TAG" ]; then
        api_tmp="$CSQTT_DIR/.releases.json"
        fetch "https://api.github.com/repos/$CSQTT_REPO/releases/latest" "$api_tmp" \
            || { rm -f "$api_tmp"; die "не удалось узнать последний релиз $CSQTT_REPO (нет сети? https-стек неполон? попробуйте: opkg install ca-bundle libustream-openssl curl)"; }
        CSQTT_TAG=$(grep -o '"tag_name": *"[^"]*"' "$api_tmp" 2>/dev/null | head -n 1 | cut -d '"' -f4)
        rm -f "$api_tmp"
        [ -n "$CSQTT_TAG" ] || die "в $CSQTT_REPO не найдено ни одного релиза"
    fi
    log "Релиз: $CSQTT_REPO $CSQTT_TAG"
    assets_tmp="$CSQTT_DIR/.assets.html"
    fetch "https://github.com/$CSQTT_REPO/releases/expanded_assets/$CSQTT_TAG" "$assets_tmp" \
        || { rm -f "$assets_tmp"; die "не удалось получить список файлов релиза"; }
    asset_url=$(grep -o 'href="[^"]*releases/download/[^"]*"' "$assets_tmp" 2>/dev/null \
        | sed 's/^href="//; s/"$//' \
...
if [ -z "$CSQTT_MIHOMO_CONF" ]; then
    # интерактивный выбор: 1) скачать 2) пропустить
    while true; do
...
if [ -z "$DEVICE_ID" ]; then
    DEVICE_ID=$(cat /sys/firmware/devicetree/base/serial-number 2>/dev/null | tr -d '\000')
...
# делает exec клиента (PID procd-инстанса = PID клиента).
# Весь вывод (обёртки и клиента) уходит в csqtt.log — его ограничивает watchdog.
...
# Случайность — только через awk srand/rand: od как источник энтропии не надёжен.
...
# OpenWrt: busybox crond читает /etc/crontabs/root
...
# Проверки: pgrep/pidof csqtt-client, ip link show csqtt0 UP (fallback —
# ifconfig), ping -I csqtt0. Любой сбой → service csqtt restart. Отличия:
#  • уважает ручную остановку — service stop создаёт stopped-флаг, watchdog ждёт;
...
#  • падения процесса ловит и сам procd (respawn), watchdog страхует TUN/пинг.
...
# ОБРЕЗКА В ТОМ ЖЕ INODE: клиент держит лог открытым (append через exec-редирект),
...
for logf in "$LOG" __LOG_FILE__; do
    [ -f "$logf" ] || continue
    FILE_SIZE=$(du -k "$logf" 2>/dev/null | awk '{print $1}')
    if [ -n "$FILE_SIZE" ] && [ "$FILE_SIZE" -gt "$MAX_SIZE_KB" ]; then
        tail -n 500 "$logf" > "${logf}.cut" 2>/dev/null \
            && cat "${logf}.cut" > "$logf" \
            && rm -f "${logf}.cut" \
            && echo "$(stamp) [WATCHDOG] Лог $logf обрезан до 500 строк." >> "$LOG"
    fi
done

# служба остановлена вручную (service csqtt stop) — не трогаем
...
if [ -n "$TUN_IFACE" ]; then
    ...
    if ip link show "$TUN_IFACE" 2>/dev/null | grep -q "UP"; then
        IS_UP=1
            ...
            && ifconfig "$TUN_IFACE" 2>/dev/null | grep -q "UP"; then
        IS_UP=1
    fi
fi
...
# CSQTT client (Авторежим ВК, пул хешей) — OpenWrt/procd
USE_PROCD=1
START=99
STOP=10

extra_command log "Последние 100 строк журнала клиента"
log() {
    tail -n 100 "$CSQTT_DIR/csqtt.log"
}
...
start_service() {
    ...
    # падения процесса перезапускает сам procd: threshold=60, timeout=5,
    # retry=0 (бесконечно) — при старте роутера дождётся и поднявшейся сети
    procd_set_param respawn 60 5 0
    # stdout/stderr обёртки уходит в csqtt.log (exec-редирект внутри);
    # эти параметры — страховка на случай раннего падения обёртки
...
stop_service() {
    # stopped-флаг: watchdog не поднимет службу, остановленную вручную
...
EOF
chmod +x "$INIT_SCRIPT"
log "Init-скрипт: $INIT_SCRIPT (procd)"

# автозапуск при загрузке роутера (симлинк в /etc/rc.d)
if "$INIT_SCRIPT" enable 2>/dev/null; then
    log "Автозапуск включён: $INIT_SCRIPT enable (START=99)"
else
    warn "Не удалось включить автозапуск — выполните вручную: $INIT_SCRIPT enable"
fi

# ── 12а. деинсталлятор (csqtt-uninstall) ─────────────────────────────────────
# Полная очистка: cron-строки ротации+watchdog, init-скрипт (disable+удаление),
# каталог установки. Пул vk_pool и VK-звонки: при удалении id звонков теряются —
# VK уберёт их сам.
mkdir -p "$(dirname "$UNINST_BIN")"
cat > "$CSQTT_DIR/uninstall.sh" <<'UNINSTALL'
#!/bin/sh
# Деинсталляция CSQTT (OpenWrt): остановить, вычистить cron, init, файлы.
DIR=$(dirname "$0")
echo "=== Удаление csqtt-client (OpenWrt) ==="
...
# автозапуск, остановка, init-скрипт
if [ -x /etc/init.d/csqtt ]; then
    /etc/init.d/csqtt disable >/dev/null 2>&1 || true
    /etc/init.d/csqtt stop >/dev/null 2>&1 || true
    rm -f /etc/init.d/csqtt
fi
rm -f /etc/rc.d/S99csqtt /etc/rc.d/K10csqtt 2>/dev/null

killall -9 csqtt-client 2>/dev/null || true
rm -f /var/run/csqtt.pid

# самоудаляющаяся обёртка и весь каталог
...
if [ "$CSQTT_START" = "1" ]; then
    ...
    sleep 4
    if pgrep csqtt-client >/dev/null 2>&1 || pidof csqtt-client >/dev/null 2>&1; then
        log "Клиент запущен, процесс жив"
    else
        warn "Процесс не поднялся за 4 с — смотрите журнал: service csqtt log"
        tail -n 10 "$LOG_FILE" 2>/dev/null
    fi
    log "Журнал: service csqtt log · системный: logread | grep csqtt"
fi

log "Готово. Управление: service csqtt start|stop|restart|status|log"
log "Журнал клиента: $LOG_FILE (watchdog держит ≤1 МБ)"
