...
elf_class_check() {
  ...
  # Важно: BusyBox od на роутерах часто НЕ поддерживает GNU/POSIX
  # ключи -t/-N/-j. Поэтому читаем ровно один байт через dd и
  # штатный octal-вывод BusyBox od.
  _f=$1
  [ -f "$_f" ] || return 1

  _c=$(dd if="$_f" bs=1 skip=4 count=1 2>/dev/null | od -An -b 2>/dev/null | tr -d ' \\t\\n\\r')
  case "$_c" in
    001) printf '32' ;;
    002) printf '64' ;;
    *)   printf '' ;;
