...
# sed -i не используем: на урезанных BusyBox-сборках (некоторые прошивки
# Keenetic) флаг -i отсутствует — портируемый вариант sed+mv работает везде.
ROTATE_INIT="$INIT_SCRIPT"
sed "s|__INIT_CMD__|$ROTATE_INIT|g" "$CSQTT_DIR/csqtt-rotate-hashes.sh" > "$CSQTT_DIR/csqtt-rotate-hashes.sh.tmp" \
    && mv "$CSQTT_DIR/csqtt-rotate-hashes.sh.tmp" "$CSQTT_DIR/csqtt-rotate-hashes.sh" \
