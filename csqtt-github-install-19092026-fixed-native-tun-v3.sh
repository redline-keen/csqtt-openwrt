...
SCRIPT_VERSION="19092026-fixed-native-tun"
DEFAULT_TAG="0.3"
...
MAX_HASHES=6
MIN_HASHES=1
MAX_WORKERS=126  # точный предел csqtt-client v2.1.9; клиент сам нормализует воркеры до 126
MIN_WORKERS=9
...
elf_magic_check() {
  # $1 = путь к файлу. Проверяем первые 4 байта ELF.
  # Никаких -A/-t/-N/-j: они отсутствуют в некоторых BusyBox od.
  _f=$1
  [ -f "$_f" ] || return 1

  _h=$(dd if="$_f" bs=1 count=4 2>/dev/null | od -b 2>/dev/null | head -1)
  case "$_h" in
    *"177 105 114 106"*) return 0 ;;
  esac
  return 1
}

elf_class_check() {
  # $1 = путь. Печатает "32"/"64"/"".
  # BusyBox-safe: od вызывается ТОЛЬКО с ключом -b.
  _f=$1
  [ -f "$_f" ] || return 1

  _c=$(dd if="$_f" bs=1 skip=4 count=1 2>/dev/null | od -b 2>/dev/null | head -1 | awk '{print $NF}')
  case "$_c" in
    001) printf '32' ;;
    002) printf '64' ;;
    *)   printf '' ;;
...
elf_machine_check() {
  ...
  # Читаем e_machine (offset 18..19) побайтно, чтобы не зависеть от
  # расширенных ключей BusyBox od.
  _f=$1
  [ -f "$_f" ] || return 1

  _d=$(dd if="$_f" bs=1 skip=5 count=1 2>/dev/null | od -b 2>/dev/null | head -1 | awk '{print $NF}')
  _b18=$(dd if="$_f" bs=1 skip=18 count=1 2>/dev/null | od -b 2>/dev/null | head -1 | awk '{print $NF}')
  _b19=$(dd if="$_f" bs=1 skip=19 count=1 2>/dev/null | od -b 2>/dev/null | head -1 | awk '{print $NF}')

  case "$_d" in
    001)
      case "$_b18$_b19" in
        267000) printf 'aarch64' ;;
        076000) printf 'x86_64' ;;
        010000) printf 'mipsel' ;;
      esac
      ;;
    002)
      case "$_b18$_b19" in
        000267) printf 'aarch64' ;;
        000076) printf 'x86_64' ;;
        000010) printf 'mips' ;;
      esac
      ;;
  esac
...
[ -n "$TUN_IFACE" ] && CLI_ARGS="$CLI_ARGS --tun $TUN_IFACE"
...
Опции:
  ...
  --workers N           число воркеров 9–126 (default: max=HASHES×27)
...
Сборка: $SCRIPT_VERSION (исправлены CLI-флаги под точный исходник csqtt-client)
EOF
}
