...
#   1341  — воркеры 9–162
#   1946  — watchdog cron */2 (процесс), uninstaller csqtt-uninstall
#   2350  — лог в $CSQTT_DIR/csqtt.log ≤1 МБ с сохранением inode, mihomo config.yaml download
#   0200  — slim csqtt.conf, device_id отдельным файлом
#   CLI-v4 — запуск только с аргументами, реально присутствующими в фактическом csqtt-client
#
...
SCRIPT_VERSION="19092026-cli-v4"
DEFAULT_TAG="0.3"
...
ask_workers() {
  if [ -z "$WORKERS_INPUT" ]; then
    if [ -t 0 ] || [ -t 1 ]; then
      printf 'Число воркеров (9–162, Enter = 108): ' >&2
      read -r _w
      WORKERS_INPUT=${_w:-108}
    else
      WORKERS_INPUT=108
    fi
  fi
  ...
  case "$WORKERS_INPUT" in
    ''|*[!0-9]*) die "воркеры должны быть числом 9–162, получено: '$WORKERS_INPUT'" ;;
  esac

  [ "$WORKERS_INPUT" -ge 9 ] && [ "$WORKERS_INPUT" -le 162 ] \
    || die "воркеры вне диапазона 9–162: $WORKERS_INPUT"

  WORKERS_REM=$((WORKERS_INPUT % 9))
  if [ "$WORKERS_REM" -ne 0 ]; then
    WORKERS_INPUT=$((WORKERS_INPUT - WORKERS_REM))
    [ "$WORKERS_INPUT" -lt 9 ] && WORKERS_INPUT=9
...
write_slim_config() {
  ...
  cat > "$CONF" <<EOF
# CSQTT — конфиг под фактический CLI csqtt-client
# OpenWrt: service csqtt restart
# Keenetic/Entware: /opt/etc/init.d/S99csqtt restart
...
FINGERPRINT="${FINGERPRINT_INPUT:-firefox}"
OBFS="${OBFS:-wrap}"
TURN_TRANSPORT="${TURN_TRANSPORT:-udp}"
CAPTCHA_MODE="${CAPTCHA_MODE:-auto}"
EOF
  chmod 600 "$CONF"
...
CLI_ARGS="$CLI_ARGS --workers $WORKERS"
...
# ВАЖНО: фактический CLI клиента не содержит --tun-iface/--tun-mtu.
# Поэтому watchdog не проверяет csqtt0 и не пытается рестартовать клиент
# только из-за отсутствия TUN.
...
Обязательный аргумент:
  CSQTT_URL            csqtt://connect?... ссылка (содержит peer/password)
...
Опции:
  ...
  --workers N           число воркеров 9–162 (default: 108)
...
Примеры:
  ...
  sh $0 'csqtt://connect?...' --repo я/csqtt-router --vk-token vk1.a.abc --workers 108
...
main() {
  ...
  write_run_wrapper
  write_init_service
  # Ротация VK-хешей отключена: фактический CLI клиента не принимает --hashes.
