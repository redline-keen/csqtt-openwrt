...
SCRIPT_VERSION="19092026-fixed-native-tun-v5"
DEFAULT_TAG="0.3"
...
MAX_HASHES=6
MIN_HASHES=1
MAX_WORKERS=126  # предел csqtt-client v2.1.9
MIN_WORKERS=9
...
elf_magic_check() {
  ...
  # В BusyBox od -b может форматировать вывод по-разному, поэтому
  # сначала пробуем BusyBox-safe od -b, затем od -x.
  _f=$1
  [ -f "$_f" ] || return 1

  _h=$(dd if="$_f" bs=1 count=4 2>/dev/null | od -An -tx1 2>/dev/null | tr -d ' \t\n\r')
  case "$_h" in
    7f454c46*) return 0 ;;
  esac

  # Запасной вариант: od -b, но разбираем отдельные байты.
  _b1=$(dd if="$_f" bs=1 skip=0 count=1 2>/dev/null | od -b 2>/dev/null | tail -1 | tr -cd '0-7')
  _b2=$(dd if="$_f" bs=1 skip=1 count=1 2>/dev/null | od -b 2>/dev/null | tail -1 | tr -cd '0-7')
  _b3=$(dd if="$_f" bs=1 skip=2 count=1 2>/dev/null | od -b 2>/dev/null | tail -1 | tr -cd '0-7')
  _b4=$(dd if="$_f" bs=1 skip=3 count=1 2>/dev/null | od -b 2>/dev/null | tail -1 | tr -cd '0-7')
...
elf_class_check() {
  # ELF EI_CLASS находится в байте 4: 1=ELF32, 2=ELF64.
  # Только BusyBox-compatible dd + od -b.
  _f=$1
  [ -f "$_f" ] || return 1

  _c=$(dd if="$_f" bs=1 skip=4 count=1 2>/dev/null |
...
elf_machine_check() {
  # ELF e_machine — u16 на offset 18.
  # Читаем два байта отдельно, чтобы не зависеть от od -t/-N/-j.
  _f=$1
  [ -f "$_f" ] || return 1
  ...
  # Little-endian:
  # AArch64 = 0x00b7 -> bytes b7 00 -> octal 267 000
  # x86_64  = 0x003e -> bytes 3e 00 -> octal 076 000
  # MIPS LE  = 0x0008 -> bytes 08 00 -> octal 010 000
  # Big-endian MIPS = 00 08 -> octal 000 010
  case "$_b18$_b19:$_d" in
    267000:1) printf 'aarch64' ;;
    076000:1) printf 'x86_64' ;;
    010000:1) printf 'mipsel' ;;
    000010:2) printf 'mips' ;;
    000008:2) printf 'mips' ;;
    *) printf '' ;;
  esac
}
...
verify_binary() {
  ...
  if ! elf_magic_check "$BIN"; then
    _raw=$(dd if="$BIN" bs=1 count=16 2>/dev/null | od -An -tx1 2>/dev/null | tr -d ' \t\n\r')
    _size=$(wc -c < "$BIN" 2>/dev/null | tr -d ' \t\n\r')
    die "файл $BIN не является корректным ELF. Первые 16 байт: ${_raw:-<не прочитаны>}; размер: ${_size:-0} байт. Значит GitHub отдал не ELF либо скачивание повреждено."
  fi
