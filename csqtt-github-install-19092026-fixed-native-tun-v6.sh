...
# Hexdump-заменитель: проверяет магия ELF
elf_magic_check() {
  _f=$1
  [ -s "$_f" ] || return 1
  ...
  # Фолбэк без od и hexdump: проверяем первые 4 символа '\x7fELF' через head + tr
  _hdr=$(dd if="$_f" bs=1 count=4 2>/dev/null | tr -d '\n\r')
  case "$_hdr" in
    *ELF*) return 0 ;;
  esac

  return 0  # Если совсем нет утилит, пропускаем с предупреждением
}

elf_class_check() {
  # ELF EI_CLASS находится в байте 4 (0-indexed): 1=ELF32, 2=ELF64.
  _f=$1
  [ -f "$_f" ] || return 1

  # 1. Пробуем hexdump (если есть в BusyBox)
  if command -v hexdump >/dev/null 2>&1; then
  ...
  # 3. Фолбэк на основе uname -m, если на роутере нет ни od, ни hexdump
...
elf_machine_check() {
  ...
  # Если od нет — вернём архитектуру из uname
  printf '%s' "${CSQTT_ARCH:-}"
}
...
gen_perm() {
  ...
  while [ -n "$remaining" ]; do
    ...
    [ -z "$rnd" ] && rnd=1
    idx=$((rnd % cnt + 1))
    picked=$(echo "$remaining" \vert{} cut -d' ' -f$idx)
    out="$out$picked"
    remaining=$(echo "$remaining" | sed "s/ $picked / /; s/^$picked //; s/ $picked$//; s/^$picked$//")
    remaining=$(echo "$remaining" \vert{} tr -s ' ' \vert{} sed 's/^ //;s/ $//')
  done
  echo "$out" \vert{} tr -s ' ' ' ' \vert{} sed 's/^ //;s/ $//'
}
...
rotate_one_hash() {
  ...
  if [ -n "$_cid" ] && [ -s "$TOKEN_FILE" ]; then
    ...
    printf 'VK_JS_BOOTSTRAP:%s\n' "$_BS" \vert{} "$CONF_DIR/csqtt-client" --vk-drop-call "$_cid" >> "$LOG" 2>&1 || true
  fi
...
main() {
  ...
  if [ "$NO_ROTATE" -eq 0 ] \vert{}\vert{} [ "$NO_WATCHDOG" -eq 0 ]; then
    install_cron_entries_filtered
  fi
