...
SCRIPT_VERSION="19092026-fixed-native-tun-v6"
DEFAULT_TAG="0.3"
...
elf_magic_check() {
  ...
  # Нормальный способ: hexdump. BusyBox hexdump это умеет.
  if command -v hexdump >/dev/null 2>&1; then
    _h=$(hexdump -n 6 -e '1/1 "%02x"' "$_f" 2>/dev/null)
    case "$_h" in
      7f454c460201) return 0 ;;  # ELF64 little-endian
      7f454c460101) return 0 ;;  # ELF32 little-endian
      7f454c460202) return 0 ;;  # ELF64 big-endian
      7f454c460102) return 0 ;;  # ELF32 big-endian
    esac
    return 1
  fi

  # Запасной вариант для систем без hexdump.
  if command -v od >/dev/null 2>&1; then
    _h=$(dd if="$_f" bs=1 count=6 2>/dev/null | od -b 2>/dev/null \
      | awk 'NR==1 {for (i=2; i<=NF; i++) {printf "%s", $i}; exit}')
    case "$_h" in
      177105114106002001) return 0 ;;
      177105114106001001) return 0 ;;
      177105114106002002) return 0 ;;
      177105114106001002) return 0 ;;
    esac
  fi
