...
# mihomo (по умолч. OpenWrt: /etc/mihomo; для SSClash укажите /opt/clash через
# --mihomo-dir); существующий конфиг сохраняется бэкапом *.csqtt-bak. Флаги:
# --mihomo-conf (скачать), --mihomo-conf-url URL, --mihomo-dir КАТАЛОГ, --no-mihomo.
#
...
CSQTT_MIHOMO_DIR=""       # каталог Mihomo; по умолч. /etc/mihomo (SSClash = /opt/clash)
CSQTT_LINK=""
WORKERS_PER_HASH=27
WORKERS_STEP=9
MAX_HASHES=4
...
MIHOMO_DIR="${CSQTT_MIHOMO_DIR:-/etc/mihomo}"
...
while :; do
    ...
    PEER_HOST=""; PEER_PORT=""; PASSWORD=""; VK_LINK_TOKEN=""
    oldIFS="$IFS"; IFS='&'
    for kv in $query; do
        k=${kv%%=*}; v=${kv#*=}
        case "$k" in
            ...
            token)    VK_LINK_TOKEN=$(urldecode "$v") ;;
        esac
    done
    IFS="$oldIFS"
...
# VK-токен из ссылки (--vk-token важнее; если задан флаг — не трогаем).
if [ -z "$CSQTT_VK_TOKEN" ] && [ -n "$VK_LINK_TOKEN" ]; then
    CSQTT_VK_TOKEN="$VK_LINK_TOKEN"
    log "VK-токен взят из ссылки подключения"
fi
...
# Неверный ввод переспрашиваем (как ссылку и токен), а не роняем установщик.
is_positive_int() { case "$1" in ""|*[!0-9]*) return 1 ;; *) [ "$1" -ge 1 ] ;; esac; }

# хеши: 1..MAX_HASHES
HASH_ATTEMPT=0
while true; do
    if is_positive_int "${CSQTT_HASHES:-}" && [ "$CSQTT_HASHES" -le "$MAX_HASHES" ]; then
        break
    fi
    HASH_ATTEMPT=$((HASH_ATTEMPT + 1))
    if [ "$HASH_ATTEMPT" -gt 1 ]; then
        warn "Некорректное число хешей (нужно целое 1..$MAX_HASHES) — попробуйте ещё раз (попытка $HASH_ATTEMPT)"
    fi
    printf 'Хешей в пуле [1..%s] (Enter = 4): ' "$MAX_HASHES"
    read -r CSQTT_HASHES || die "не удалось прочитать число хешей (нет stdin)"
    [ -n "$CSQTT_HASHES" ] || CSQTT_HASHES=4
done

# воркеры: 9..162, максимум хеши×27, кратно 9 (неверное — переспрашиваем)
hash_cap=$((CSQTT_HASHES * WORKERS_PER_HASH))
WORKER_ATTEMPT=0
while true; do
    if is_positive_int "${CSQTT_WORKERS:-}"; then
        [ "$CSQTT_WORKERS" -ge 9 ] && break
        warn "Минимум 9 воркеров (попытка $WORKER_ATTEMPT)"
    elif [ -n "${CSQTT_WORKERS:-}" ]; then
        WORKER_ATTEMPT=$((WORKER_ATTEMPT + 1))
        warn "Число воркеров должно быть целым (попытка $WORKER_ATTEMPT)"
    else
        CSQTT_WORKERS=$hash_cap
        break
    fi
    WORKER_ATTEMPT=$((WORKER_ATTEMPT + 1))
    printf 'Воркеров [9..%d, максимум %d для %d хешей] (Enter = максимум): ' \
        "$(( MAX_HASHES * WORKERS_PER_HASH ))" "$hash_cap" "$CSQTT_HASHES"
    read -r CSQTT_WORKERS || die "не удалось прочитать число воркеров (нет stdin)"
done
...
# Источник по убыванию предпочтения: сохранённый файл → серийник платы →
# MAC интерфейса → /etc/machine-id → случайный hex (один раз, потом не меняется).
# Прежняя версия звала `od` (нет в busybox) и склеивала пустые hostname+hex в «-».
...
if [ -z "$DEVICE_ID" ]; then
    DEVICE_ID=$(sanitize_id "$(cat /sys/firmware/devicetree/base/serial-number 2>/dev/null | tr -d '\0')")
    [ -n "$DEVICE_ID" ] || DEVICE_ID=$(sanitize_id "$(cat /etc/serial 2>/dev/null)")
    [ -n "$DEVICE_ID" ] || DEVICE_ID=$(sanitize_id "$(first_mac)")
    [ -n "$DEVICE_ID" ] || DEVICE_ID=$(sanitize_id "$(head -c 32 /etc/machine-id 2>/dev/null)")
    [ -n "$DEVICE_ID" ] || DEVICE_ID="csqtt-$(random_hex)"
