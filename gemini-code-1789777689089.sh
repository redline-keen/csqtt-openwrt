...
if [ "$CSQTT_START" = "1" ]; then
    if [ "$INIT_STYLE" = "openwrt" ]; then
        "$INIT_SCRIPT" enable
        "$INIT_SCRIPT" restart
        log "Служба добавлена в автозагрузку OpenWrt."
        log "Журнал (системный): logread | grep csqtt"
        log "Файл лога: $LOG_FILE"
    else
        "$INIT_SCRIPT" restart
        sleep 4
...
log "Готово. Управление: $INIT_SCRIPT start|stop|restart|status"
[ "$INIT_STYLE" = "openwrt" ] && log "Управление (OpenWrt): service csqtt start|stop|restart"
log "VK-токен: $VK_TOKEN_FILE · конфиг: $CSQTT_DIR/csqtt.conf · лог: $LOG_FILE · пул: $CSQTT_DIR/vk_pool"
