...
# Версия v9: исправление форматирования CLI_ARGS (пробелы около --password, удаление --hashes)
...
CMD_ARGS="$CMD_ARGS --vk-hash-mode auto_js"
CMD_ARGS="$CMD_ARGS --vk-auth-mode auto_js"
CMD_ARGS="$CMD_ARGS --allow-hash-redistribution"
CMD_ARGS="$CMD_ARGS --device-id \"$DEVICE_ID_VAL\""
CMD_ARGS="$CMD_ARGS --workers \"$WORKERS\""
CMD_ARGS="$CMD_ARGS --listen \"$LISTEN_D\""
CMD_ARGS="$CMD_ARGS --fingerprint \"$FINGERPRINT_D\""
CMD_ARGS="$CMD_ARGS --obfs \"$OBFS_D\""
CMD_ARGS="$CMD_ARGS --turn-transport \"$TURN_TRANSPORT_D\""
CMD_ARGS="$CMD_ARGS --captcha-mode \"$CAPTCHA_MODE_D\""

if [ -n "$CLIENT_IDS_D" ]; then
  CMD_ARGS="$CMD_ARGS --client-ids \"$CLIENT_IDS_D\""
fi
...
} | eval "\"$CONF_DIR/csqtt-client\" $CMD_ARGS" >> "$LOG_FILE" 2>&1
EOF_RUN
  chmod +x "$RUN"
