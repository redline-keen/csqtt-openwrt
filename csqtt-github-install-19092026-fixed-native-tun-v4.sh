...
SCRIPT_VERSION="19092026-fixed-native-tun-v4"
DEFAULT_TAG="0.3"
...
elf_magic_check() {
  # Проверяем ELF magic без GNU-ключей od/hexdump.
  _f=$1
  [ -f "$_f" ] || return 1

  _b1=$(dd if="$_f" bs=1 skip=0 count=1 2>/dev/null | od -b 2>/dev/null | tr -d ' \t\n\r')
  _b2=$(dd if="$_f" bs=1 skip=1 count=1 2>/dev/null | od -b 2>/dev/null | tr -d ' \t\n\r')
  _b3=$(dd if="$_f" bs=1 skip=2 count=1 2>/dev/null | od -b 2>/dev/null | tr -d ' \t\n\r')
  _b4=$(dd if="$_f" bs=1 skip=3 count=1 2>/dev/null | od -b 2>/dev/null | tr -d ' \t\n\r')

  [ "$_b1$_b2$_b3$_b4" = "177105114106" ]
}
