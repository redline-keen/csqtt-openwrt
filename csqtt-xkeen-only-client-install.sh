...
# Современные VK-токены выглядят как "vk1.a.<base64url-пейлоад>" (точки в префиксе —
# это часть формата, не опечатка), например:
#   vk1.a.EMl1l-f0CyW7wgfpaFHtU9X4fw9goW9x7iU4UCCxLbSBuXS3w75mDpuvUXKTHHNoLC6LMINwNCuA8...
# Старые токены — просто ~85-символьная hex/base64-строка без точек. Поддерживаем оба варианта.
vk_token_is_valid() {
    _t="$1"
    [ -n "$_t" ] || return 1
    case "$_t" in
        ...
        *..*) return 1 ;;   # две точки подряд — точно не токен
        .*|*.) return 1 ;;  # точка в начале/конце — тоже не похоже на токен
    esac
    [ "${#_t}" -ge 50 ] || return 1
    return 0
}
